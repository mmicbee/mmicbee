<!-- Millicent Odhiambo — GitHub Profile README -->

<div align="center">

```
╔══════════════════════════════════════╗
║  MMICBEE://ENGINEER_PROFILE          ║
║  STATUS: ONLINE                      ║
║  ROLE: BACKEND SYSTEMS APPRENTICE    ║
║  NODE: KISUMU_01                     ║
╚══════════════════════════════════════╝
```

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=800&color=00E5FF&center=true&vCenter=true&width=650&lines=Millicent+Odhiambo+%7C+%40mmicbee;Backend+Systems+Apprentice;AI+%2F+Blockchain+%2F+Infrastructure;Build.+Break.+Learn.)](https://git.io/typing-svg)

<img src="https://komarev.com/ghpvc/?username=mmicbee&style=flat-square&color=00e5ff&label=PROFILE+VIEWS" alt="profile views"/>

</div>

---

```txt
> loading profile...

I build to understand how things work.

Currently training at Zone01 Kisumu,
exploring backend systems, infrastructure,
and the technologies shaping the future.

My curiosity usually leads me into:
→ AI systems
→ blockchain networks
→ backend architecture
→ whatever breaks at 2AM

status: continuously learning...
```

---

## ◈ CORE SYSTEMS

<div align="center">

![Python](https://img.shields.io/badge/Python-111827?style=for-the-badge&logo=python&logoColor=00e5ff)
![Go](https://img.shields.io/badge/Go-111827?style=for-the-badge&logo=go&logoColor=00e5ff)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-111827?style=for-the-badge&logo=postgresql&logoColor=00e5ff)
![Redis](https://img.shields.io/badge/Redis-111827?style=for-the-badge&logo=redis&logoColor=00e5ff)
![Docker](https://img.shields.io/badge/Docker-111827?style=for-the-badge&logo=docker&logoColor=bd00ff)
![Linux](https://img.shields.io/badge/Linux-111827?style=for-the-badge&logo=linux&logoColor=bd00ff)
![Git](https://img.shields.io/badge/Git-111827?style=for-the-badge&logo=git&logoColor=bd00ff)
![REST APIs](https://img.shields.io/badge/REST_APIs-111827?style=for-the-badge&logo=fastapi&logoColor=bd00ff)

</div>

---

## ◈ ACTIVE MISSIONS

```
MISSION_001 :: BACKEND SYSTEMS

[x] APIs
[x] Databases
[x] Server architecture
[ ] Distributed systems
[ ] Microservices


MISSION_002 :: ARTIFICIAL INTELLIGENCE

[x] Exploring AI concepts
[ ] Building intelligent systems
[ ] AI-powered applications


MISSION_003 :: DECENTRALIZED FUTURES

[ ] Blockchain experiments
[ ] Web3 infrastructure
[ ] Smart contracts
```

---

## ◈ CURRENT SIGNAL

```yaml
identity: Millicent Odhiambo
alias:    mmicbee
node:     Zone01 Kisumu
role:     backend engineer in progress

currently:
  - building real projects
  - learning system design
  - exploring AI + blockchain
  - improving every iteration

philosophy:
  "I chase problems where the documentation ends."
```

---

## ◈ EXPERIMENT ARCHIVE

```
Projects are not just code.
They are experiments.

Every build answers a question:
→ How does this scale?
→ How does this fail?
→ How can this be better?

[ repositories below ↓ ]
```

---

## ◈ SYSTEM LOGS

<div align="center">

<img height="180" src="https://github-readme-stats.vercel.app/api?username=mmicbee&show_icons=true&theme=synthwave&hide_border=true&count_private=true" />

<img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=mmicbee&layout=compact&theme=synthwave&hide_border=true" />

</div>

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=mmicbee&theme=synthwave&hide_border=true)](https://git.io/streak-stats)

</div>

---

## ◈ ACTIVITY SCAN

[![Millicent's github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=mmicbee&theme=synthwave&hide_border=true)](https://github.com/ashutosh00710/github-readme-activity-graph)

---

## ◈ NETWORK CONNECTION

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-111827?style=for-the-badge&logo=linkedin&logoColor=00e5ff)](https://linkedin.com)
[![Email](https://img.shields.io/badge/Email-111827?style=for-the-badge&logo=gmail&logoColor=bd00ff)](mailto:millibecky4@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-111827?style=for-the-badge&logo=github&logoColor=00e5ff)](https://github.com/mmicbee)

</div>

---

<div align="center">

```
╭──────────────────────────────╮
│  BUILD  //  BREAK  //  LEARN  │
╰──────────────────────────────╯
```

`connection terminated: profile loaded successfully`

</div>
